...
var WorkboxSW = (function() {
  ...
  const errors = {
    ...
    "unsupported-route-type":
      "The first parameter to registerRoute() should be either an Express-style path string, a RegExp, or a function.",
  ...
  function normalizeHandler(a) {
    return "object" == typeof a
      ? (hasMethod({ handler: a }, "handle"), a)
      : (isType({ handler: a }, "function"), { handle: a });
  }
  ...
  class Route {
    constructor({ match: a, handler: b, method: c } = {}) {
      (this.handler = normalizeHandler(b)),
        isType({ match: a }, "function"),
        (this.match = a),
        c
          ? (isOneOf({ method: c }, validMethods), (this.method = c))
  ...
  function parse(a, b) {
    for (
      var c, d = [], e = 0, f = 0, g = "", h = (b && b.delimiter) || "/";
      null != (c = PATH_REGEXP.exec(a));

    ) {
      var i = c[0],
        j = c[1],
        k = c.index;
      if (((g += a.slice(f, k)), (f = k + i.length), j)) {
        g += j[1];
        continue;
      }
      var l = a[f],
      ...
      d.push({
        ...
        optional: "?" === q || "*" === q,
        repeat: "+" === q || "*" === q,
        partial: null != m && null != l && l !== m,
        asterisk: !!r,
        pattern: t ? escapeGroup(t) : r ? ".*" : "[^" + escapeString(s) + "]+?"
      });
    }
    ...
    return function(c, d) {
      for (
        var e,
          ...
          g = c || {},
          h = d || {},
          k = h.pretty ? encodeURIComponentPretty : encodeURIComponent,
          l = 0;
        l < a.length;
        l++
      ) {
        ...
        if (null == m)
          if (e.optional) {
            e.partial && (f += e.prefix);
            continue;
          } else throw new TypeError('Expected "' + e.name + '" to be defined');
        if (index$1(m)) {
          if (!e.repeat)
            throw new TypeError(
              'Expected "' +
          ...
          for (var n = 0; n < m.length; n++) {
            ...
            f += (0 === n ? e.prefix : e.delimiter) + i;
          }
          continue;
        }
        if (((i = e.asterisk ? encodeAsterisk(m) : k(m)), !b[l].test(i)))
          throw new TypeError(
            'Expected "' +
              e.name +
              '" to match "' +
  ...
  function arrayToRegexp(a, b, c) {
    ...
    var f = new RegExp("(?:" + d.join("|") + ")", flags(c));
    return attachKeys(f, b);
  }
  function stringToRegexp(a, b, c) {
    return tokensToRegExp(parse(a, c), b, c);
  }
  function tokensToRegExp(a, b, c) {
    index$1(b) || ((c = b || c), (b = [])), (c = c || {});
    for (
      var d, e = c.strict, f = !1 !== c.end, g = "", h = 0;
      h < a.length;
      h++
    )
      if (((d = a[h]), "string" == typeof d)) g += escapeString(d);
      else {
    ...
    var k = escapeString(c.delimiter || "/"),
      l = g.slice(-k.length) === k;
    return (
      e || (g = (l ? g.slice(0, -k.length) : g) + "(?:" + k + "(?=$))?"),
      (g += f ? "$" : e && l ? "" : "(?=" + k + "|$)"),
      attachKeys(new RegExp("^" + g, flags(c)), b)
    );
  }
  function pathToRegexp(a, b, c) {
    return (
      ...
      a instanceof RegExp
        ? regexpToRegexp(a, b)
        : index$1(a) ? arrayToRegexp(a, b, c) : stringToRegexp(a, b, c)
    );
  }
  (index.parse = parse_1),
  ...
  class ExpressRoute extends Route {
    constructor({ path: a, handler: b, method: c }) {
      ...
      super({
        match: ({ url: b }) => {
          if (a.startsWith("/") && b.origin !== location.origin) return null;
          const c = a.startsWith("/") ? b.pathname : b.href,
            f = c.match(e);
          if (!f) return null;
          const g = {};
          return (
  ...
  class LogGroup {
    ...
    print() {
      return 0 === this._logs.length && 0 === this._childGroups.length
        ? void this._printLogDetails(this._primaryLog)
        : void (this._primaryLog &&
            (this._isFallbackMode
              ? this._printLogDetails(this._primaryLog)
              : console.groupCollapsed(
                  ...this._getLogContent(this._primaryLog)
                )),
    ...
    _getLogContent(a) {
      ...
      let c = [b];
      return (
        !this._isFallbackMode && a.colors && (c = c.concat(a.colors)),
        a.args && (c = c.concat(a.args)),
        c
      );
    }
  }
  ...
  class LogHelper {
    ...
    _getAllLogGroups(a, b) {
      ...
      if ((c.addPrimaryLog(d), b.error)) {
        const a = { message: b.error, logFunc: console.error };
        c.addLog(a);
      }
      const e = new LogGroup();
      if (b.that && b.that.constructor && b.that.constructor.name) {
        const a = b.that.constructor.name;
        e.addLog(this._getKeyValueDetails("class", a));
      }
      return (
        b.data &&
          ("object" != typeof b.data || b.data instanceof Array
            ? e.addLog(this._getKeyValueDetails("additionalData", b.data))
            : Object.keys(b.data).forEach(a => {
                e.addLog(this._getKeyValueDetails(a, b.data[a]));
              })),
        c.addChildGroup(e),
        c
      );
    ...
    _getPrimaryMessageDetails(a, b) {
      let c, d;
      a === self.workbox.LOG_LEVEL.verbose
        ? ((c = "Info"), (d = LIGHT_GREY))
        : a === self.workbox.LOG_LEVEL.debug
          ? ((c = "Debug"), (d = LIGHT_GREEN))
          : a === self.workbox.LOG_LEVEL.warn
            ? ((c = "Warn"), (d = LIGHT_YELLOW))
            : a === self.workbox.LOG_LEVEL.error
              ? ((c = "Error"), (d = LIGHT_RED))
              : void 0;
      let e = `%c🔧 %c[${c}]`;
    ...
    _shouldLogMessage(a, b) {
      ...
      return (
        self &&
          self.workbox &&
          "number" == typeof self.workbox.logLevel &&
          (c = self.workbox.logLevel),
        c === self.workbox.LOG_LEVEL.none || a < c ? !1 : !0
      );
    }
  }
  ...
  class NavigationRoute extends Route {
    constructor({ whitelist: a, blacklist: b, handler: c } = {}) {
      isArrayOfClass({ whitelist: a }, RegExp),
        b ? isArrayOfClass({ blacklist: b }, RegExp) : (b = []);
      super({
        match: ({ event: d, url: e }) => {
          let f,
            g = !1;
          if ("navigate" === d.request.mode) {
            const d = e.pathname + e.search;
            a.some(a => a.test(d))
              ? b.some(a => a.test(d))
                ? (f =
                    `The navigation route is not being used, since the ` +
                    `request URL matches both the whitelist and blacklist.`)
                : ((f = `The navigation route is being used.`), (g = !0))
              : (f =
                  `The navigation route is not being used, since the ` +
  ...
  class RegExpRoute extends Route {
    constructor({ regExp: a, handler: b, method: c }) {
      ...
      super({
        match: ({ url: b }) => {
          const c = a.exec(b.href);
          return c
            ? b.origin !== location.origin && 0 !== c.index
              ? (logHelper.debug({
                  that: this,
                  message:
                    `Skipping route, because the RegExp match didn't occur ` +
                    `at the start of the URL.`,
                  data: { url: b.href, regExp: a }
                }),
                null)
              : c.slice(1)
            : null;
  ...
  class Router$2 {
    ...
    addFetchListener() {
      return this._isListenerRegistered
        ? (logHelper.warn({
            that: this,
            message: `addFetchListener() has already been called for this Router.`
          }),
          !1)
        : ((this._isListenerRegistered = !0),
          self.addEventListener("fetch", a => {
            const b = this.handleRequest({ event: a });
            b && a.respondWith(b);
          }),
          !0);
    }
    handleRequest({ event: a }) {
      ...
      let { handler: c, params: d } = this._findHandlerAndParams({
        event: a,
        url: b
      });
      if ((!c && this.defaultHandler && (c = this.defaultHandler), c)) {
        let e = c.handle({ url: b, event: a, params: d });
        return (
          this.catchHandler &&
            (e = e.catch(c =>
              this.catchHandler.handle({ url: b, event: a, error: c })
            )),
          e
        );
      }
    }
    _findHandlerAndParams({ event: a, url: b }) {
      const c = this._routes.get(a.request.method) || [];
      for (const d of c) {
        let c = d.match({ url: b, event: a });
        if (c)
          return (
            logHelper.log({
              that: this,
    ...
    unregisterRoutes({ routes: a } = {}) {
      ...
      for (let b of a) {
        ...
        const a = this._routes.get(b.method).indexOf(b);
        -1 < a
          ? this._routes.get(b.method).splice(a, 1)
          : logHelper.error({
              that: this,
  ...
  class Router$$1 extends Router$2 {
    ...
    registerRoute(a, b, c = "GET") {
      "function" == typeof b && (b = { handle: b });
      let d;
      if ("string" == typeof a) {
        if (0 === a.length)
          throw ErrorFactory.createError("empty-express-string");
        d = new ExpressRoute({ path: a, handler: b, method: c });
      } else if (a instanceof RegExp)
        d = new RegExpRoute({ regExp: a, handler: b, method: c });
      else if ("function" == typeof a)
        d = new Route({ match: a, handler: b, method: c });
      else throw ErrorFactory.createError("unsupported-route-type");
      return super.registerRoute({ route: d }), d;
    }
  ...
  const errors$2 = {
    ...
    "multiple-cached-response-will-be-used-plugins":
      "You cannot register more than one plugin that implements cachedResponseWillBeUsed.",
  ...
  class CacheableResponse {
    constructor({ statuses: a, headers: b } = {}) {
      atLeastOne({ statuses: a, headers: b }),
        a !== void 0 && isArrayOfType({ statuses: a }, "number"),
        b !== void 0 && isType({ headers: b }, "object"),
        (this.statuses = a),
        (this.headers = b);
    }
    isResponseCacheable({ request: a, response: b } = {}) {
      ...
      if (
        ...
        this.headers &&
          c &&
          (c = Object.keys(this.headers).some(
            a => b.headers.get(a) === this.headers[a]
          )),
        !c)
      ) {
        const c = { response: b };
        this.statuses &&
          ...
          this.headers && (c["valid-headers"] = JSON.stringify(this.headers)),
          a && (c.request = a),
          logHelper.debug({
  ...
  class RequestWrapper {
    constructor(
      ...
      a
        ...
        d && (isType({ fetchOptions: d }, "object"), (this.fetchOptions = d)),
        e && (isType({ matchOptions: e }, "object"), (this.matchOptions = e)),
        (this.plugins = new Map()),
        c &&
          (isArrayOfType({ plugins: c }, "object"),
          c.forEach(a => {
            for (let b of pluginCallbacks)
              if ("function" == typeof a[b]) {
                if (!this.plugins.has(b)) this.plugins.set(b, []);
                else if ("cacheWillUpdate" === b)
                  throw ErrorFactory$4.createError(
                    "multiple-cache-will-update-plugins"
                  );
                else if ("cachedResponseWillBeUsed" === b)
                  throw ErrorFactory$4.createError(
                    "multiple-cached-response-will-be-used-plugins"
                  );
                this.plugins.get(b).push(a);
              }
          })),
        this.plugins.has("cacheWillUpdate") &&
          (this._userSpecifiedCachableResponsePlugin = this.plugins.get(
            "cacheWillUpdate"
          )[0]);
    }
    ...
    getCache() {
      ...
      return asyncToGenerator(function*() {
        return (
          a._cache || (a._cache = yield caches.open(a.cacheName)), a._cache
        );
      })();
    }
    match({ request: a }) {
      ...
      return asyncToGenerator(function*() {
        ...
        const c = yield b.getCache();
        let d = yield c.match(a, b.matchOptions);
        if (b.plugins.has("cachedResponseWillBeUsed")) {
          const e = b.plugins.get("cachedResponseWillBeUsed")[0];
          d = yield e.cachedResponseWillBeUsed({
            request: a,
            cache: c,
            cachedResponse: d,
    ...
    fetch({ request: a }) {
      ...
      return asyncToGenerator(function*() {
        "string" == typeof a
          ? (a = new Request(a))
          : isInstance({ request: a }, Request);
        const c = b.plugins.has("fetchDidFail") ? a.clone() : null;
        if (b.plugins.has("requestWillFetch"))
          for (let c of b.plugins.get("requestWillFetch")) {
            const b = yield c.requestWillFetch({ request: a });
            isInstance({ returnedRequest: b }, Request), (a = b);
          }
        try {
          return yield fetch(a, b.fetchOptions);
        } catch (a) {
          if (b.plugins.has("fetchDidFail"))
            for (let a of b.plugins.get("fetchDidFail"))
              yield a.fetchDidFail({ request: c.clone() });
          throw a;
        }
      })();
    }
    fetchAndCache({
      ...
      return asyncToGenerator(function*() {
        atLeastOne({ request: a });
        let g;
        const h = yield f.fetch({ request: a }),
          i =
            f._userSpecifiedCachableResponsePlugin ||
            d ||
            f.getDefaultCacheableResponsePlugin(),
          j = yield i.cacheWillUpdate({ request: a, response: h });
        if (j) {
          const b =
            e && h.redirected
              ? yield cleanResponseCopy({ response: h })
              : h.clone();
          g = f.getCache().then(
            (() => {
              var d = asyncToGenerator(function*(d) {
                let e;
                const g = c || a;
                if (
                  ("opaque" !== h.type &&
                    f.plugins.has("cacheDidUpdate") &&
                    (e = yield f.match({ request: g })),
                  yield d.put(g, b),
                  f.plugins.has("cacheDidUpdate"))
                )
                  for (let a of f.plugins.get("cacheDidUpdate"))
                    yield a.cacheDidUpdate({
                      cacheName: f.cacheName,
                      oldResponse: e,
                      newResponse: b,
                      url: "url" in g ? g.url : g
                    });
              });
              return function() {
        ...
        } else if (!j && b) throw ErrorFactory$4.createError("invalid-response-for-caching");
        return b && g && (yield g), h;
      })();
    }
  }
  ...
  class CacheFirst extends Handler {
    handle({ event: a } = {}) {
      ...
      return asyncToGenerator(function*() {
        isInstance({ event: a }, FetchEvent);
        const c = yield b.requestWrapper.match({ request: a.request });
        return (
          c ||
          (yield b.requestWrapper.fetchAndCache({
            request: a.request,
            waitOnCache: b.waitOnCache
          }))
        );
      })();
    }
  ...
  class CacheOnly extends Handler {
    handle({ event: a } = {}) {
      ...
      return asyncToGenerator(function*() {
        return (
          isInstance({ event: a }, FetchEvent),
          yield b.requestWrapper.match({ request: a.request })
        );
      })();
    }
  }
  ...
  class NetworkFirst extends Handler {
    constructor(a = {}) {
      ...
      b &&
        (isType({ networkTimeoutSeconds: b }, "number"),
    ...
    handle({ event: a } = {}) {
      ...
      return asyncToGenerator(function*() {
        ...
        const e = b.requestWrapper
          .fetchAndCache({
            request: a.request,
            waitOnCache: b.waitOnCache,
            cacheResponsePlugin: b._cacheablePlugin
          })
          .then(function(a) {
            return (
              d && clearTimeout(d),
        ...
        return c.push(e), Promise.race(c);
      })();
    }
  }
  ...
  class NetworkOnly extends Handler {
    handle({ event: a } = {}) {
      ...
      return asyncToGenerator(function*() {
        return (
          isInstance({ event: a }, FetchEvent),
          yield b.requestWrapper.fetch({ request: a.request })
        );
      })();
    }
  }
    ...
    handle({ event: a } = {}) {
      ...
      return asyncToGenerator(function*() {
        ...
        const c = b.requestWrapper
          ...
          d = yield b.requestWrapper.match({ request: a.request });
        return d || (yield c);
      })();
    }
  }
  ...
  class CacheExpiration {
    ...
    getDB({ cacheName: a } = {}) {
      ...
      return asyncToGenerator(function*() {
        ...
        const c = `${idbName}-${a}`;
        if (!b._dbs.has(c)) {
          const d = yield idb.open(c, idbVersion, function(b) {
            const c = b.createObjectStore(a, { keyPath: urlPropertyName });
            c.createIndex(timestampPropertyName, timestampPropertyName, {
              unique: !1
            });
          });
          b._dbs.set(c, d);
    ...
    getCache({ cacheName: a } = {}) {
      var b = this;
      return asyncToGenerator(function*() {
        if ((isType({ cacheName: a }, "string"), !b._caches.has(a))) {
          const c = yield caches.open(a);
          b._caches.set(a, c);
        }
        return b._caches.get(a);
      })();
    }
    isResponseFresh({ cacheName: a, cachedResponse: b, now: c } = {}) {
      if (b && this.maxAgeSeconds) {
        isInstance({ cachedResponse: b }, Response);
        const d = b.headers.get("date");
        if (d) {
          "undefined" == typeof c && (c = Date.now());
          const a = new Date(d),
            b = a.getTime();
          return !!isNaN(b) || b + 1e3 * this.maxAgeSeconds > c;
        }
    ...
    updateTimestamp({ cacheName: a, url: b, now: c } = {}) {
      var d = this;
      return asyncToGenerator(function*() {
        isType({ url: b }, "string"), isType({ cacheName: a }, "string");
        const e = new URL(b, location);
        (e.hash = ""), "undefined" == typeof c && (c = Date.now());
        const f = yield d.getDB({ cacheName: a }),
          g = f.transaction(a, "readwrite");
        g
          .objectStore(a)
          .put({ [timestampPropertyName]: c, [urlPropertyName]: e.href }),
          yield g.complete;
      })();
    }
    expireEntries({ cacheName: a, now: b } = {}) {
      var c = this;
      return asyncToGenerator(function*() {
        if (c._expirationMutex) return void (c._timestampForNextRun = b);
        (c._expirationMutex = !0),
          isType({ cacheName: a }, "string"),
          "undefined" == typeof b && (b = Date.now());
        const d = c.maxAgeSeconds
            ? yield c.findOldEntries({ cacheName: a, now: b })
            : [],
          e = c.maxEntries ? yield c.findExtraEntries({ cacheName: a }) : [],
          f = [...new Set(d.concat(e))];
        if (
          (yield c.deleteFromCacheAndIDB({ cacheName: a, urls: f }),
          0 < f.length &&
            logHelper.debug({
              that: c,
              message: "Expired entries have been removed from the cache.",
              data: { cacheName: a, urls: f }
            }),
          (c._expirationMutex = !1),
          c._timestampForNextRun)
        ) {
          const b = c._timestampForNextRun;
          return (
            (c._timestampForNextRun = null),
            c.expireEntries({ cacheName: a, now: b })
          );
        }
      })();
    }
    findOldEntries({ cacheName: a, now: b } = {}) {
      var c = this;
      return asyncToGenerator(function*() {
        isType({ cacheName: a }, "string"), isType({ now: b }, "number");
        const d = b - 1e3 * c.maxAgeSeconds,
          e = [],
        ...
        return (
          i.iterateCursor(function(a) {
            a &&
              (a.value[timestampPropertyName] < d &&
                e.push(a.value[urlPropertyName]),
              a.continue());
          }),
          yield g.complete,
          e
    ...
    findExtraEntries({ cacheName: a } = {}) {
      ...
      return asyncToGenerator(function*() {
        isType({ cacheName: a }, "string");
        const c = [],
          d = yield b.getDB({ cacheName: a });
        let e = d.transaction(a, "readonly"),
          f = e.objectStore(a),
          g = f.index(timestampPropertyName);
        const h = yield g.count();
        return (
          h > b.maxEntries &&
            ((e = d.transaction(a, "readonly")),
            (f = e.objectStore(a)),
            (g = f.index(timestampPropertyName)),
            g.iterateCursor(function(a) {
              a &&
                (c.push(a.value[urlPropertyName]),
                h - c.length > b.maxEntries && a.continue());
            })),
          yield e.complete,
          c
        );
    ...
    deleteFromCacheAndIDB({ cacheName: a, urls: b } = {}) {
      ...
      return asyncToGenerator(function*() {
        if (
          (isType({ cacheName: a }, "string"),
          isArrayOfType({ urls: b }, "string"),
          0 < b.length)
        ) {
          const d = yield c.getCache({ cacheName: a }),
            e = yield c.getDB({ cacheName: a });
          for (let c of b) {
            yield d.delete(c);
            const b = e.transaction(a, "readwrite"),
              f = b.objectStore(a);
            f.delete(c), yield b.complete;
          }
        }
      })();
    }
  ...
  class CacheExpirationPlugin extends CacheExpiration {
    cachedResponseWillBeUsed({ cacheName: a, cachedResponse: b, now: c } = {}) {
      return this.isResponseFresh({ cacheName: a, cachedResponse: b, now: c })
        ? b
        : null;
    }
    cacheDidUpdate({ cacheName: a, newResponse: b, url: c, now: d } = {}) {
      var e = this;
      return asyncToGenerator(function*() {
        isType({ cacheName: a }, "string"),
          isInstance({ newResponse: b }, Response),
          "undefined" == typeof d && (d = Date.now()),
          yield e.updateTimestamp({ cacheName: a, url: c, now: d }),
          yield e.expireEntries({ cacheName: a, now: d });
      })();
    }
  }
  ...
  function broadcastUpdate(
    { channel: a, cacheName: b, url: c, source: d } = {}
  ) {
    isInstance({ channel: a }, BroadcastChannel),
      isType({ cacheName: b }, "string"),
      isType({ source: d }, "string"),
      isType({ url: c }, "string"),
      a.postMessage({
  ...
  function responsesAreSame({ first: a, second: b, headersToCheck: c } = {}) {
    ...
    const d = c.some(c => a.headers.has(c) && b.headers.has(c));
    return d
      ? c.every(
          c =>
  ...
  class BroadcastCacheUpdatePlugin extends BroadcastCacheUpdate {
    cacheDidUpdate({ cacheName: a, oldResponse: b, newResponse: c, url: d }) {
      isType({ cacheName: a }, "string"),
        isInstance({ newResponse: c }, Response),
        b &&
          this.notifyIfUpdated({ cacheName: a, first: b, second: c, url: d });
    }
  }
  ...
  class Strategies {
    ...
    _getCachingMechanism(a, b = {}) {
      const c = {
        ...
        d = { plugins: [] };
      b.excludeCacheId || (d.cacheId = this._cacheId),
      ...
      return (
        ...
        (b.requestWrapper = new RequestWrapper(d)),
        new a(b)
      );
    }
  }
  ...
  class WorkboxError extends Error {
    constructor(a, b) {
      super(),
        (this.name = a),
        (this.message = errorMessageFactory(a, b)),
        b && (this.extras = b);
    }
  }
  ...
  class BaseCacheManager {
    constructor({ cacheName: a, cacheId: b, plugins: c } = {}) {
      if (b && ("string" != typeof b || 0 === b.length))
        throw new WorkboxError("bad-cache-id", { cacheId: b });
    ...
    _addEntries(a) {
      (this._parsedCacheUrls = null),
        a.forEach(a => {
          this._addEntryToInstallList(this._parseEntry(a));
        });
    }
    getCacheName() {
    ...
    install() {
      var a = this;
      return asyncToGenerator(function*() {
        if (0 === a._entriesToCache.size) return [];
        const b = [];
        return (
          a._entriesToCache.forEach(function(c) {
    ...
    _cacheEntry(a) {
      ...
      return asyncToGenerator(function*() {
        ...
        try {
          return (
            yield b._requestWrapper.fetchAndCache({
              request: a.getNetworkRequest(),
              waitOnCache: !0,
              cacheKey: a.request,
              cleanRedirects: !0
            }),
            yield b._onEntryCached(a),
            d
          );
        } catch (b) {
    ...
    cleanup() {
      var a = this;
      return asyncToGenerator(function*() {
        if (!(yield caches.has(a.getCacheName()))) return;
        const b = [];
        a._entriesToCache.forEach(function(a) {
          b.push(a.request.url);
        });
        const c = yield a._getCache(),
          d = yield c.keys(),
        ...
        return Promise.all(
          e.map(
            (() => {
              var b = asyncToGenerator(function*(b) {
                yield c.delete(b), yield a._onEntryDeleted(b.url);
              });
              return function() {
    ...
    _getCache() {
      ...
      return asyncToGenerator(function*() {
        return (
          a._cache || (a._cache = yield caches.open(a.getCacheName())), a._cache
        );
      })();
    }
    _parseEntry() {
  ...
  class StringCacheEntry extends BaseCacheEntry {
    constructor(a) {
      if ((isType({ url: a }, "string"), 0 === a.length))
        throw new WorkboxError("invalid-string-entry", { url: a });
      super({
        entryID: a,
        revision: a,
  ...
  class ObjectCacheEntry extends BaseCacheEntry {
    constructor({ entryID: a, revision: b, url: c, cacheBust: d }) {
      if (
        "undefined" != typeof b &&
        (isType({ revision: b }, "string"), 0 === b.length)
      )
      ...
      if ("undefined" == typeof a) a = new URL(c, location).toString();
      else if (0 === a.length)
        throw new WorkboxError("invalid-object-entry", {
  ...
  class RevisionedCacheManager extends BaseCacheManager {
    ...
    addToCacheList({ revisionedFiles: a } = {}) {
      isInstance({ revisionedFiles: a }, Array), super._addEntries(a);
      const b = a.filter(a => "string" == typeof a || !a.revision);
      0 < b.length &&
        logHelper.debug({
          that: this,
    ...
    _parseEntry(a) {
      if (null === a)
        throw new WorkboxError("unexpected-precache-entry", { input: a });
      let b;
      switch (typeof a) {
        case "string":
    ...
    _onDuplicateInstallEntryFound(a, b) {
      if (b.revision !== a.revision)
        throw new WorkboxError("duplicate-entry-diff-revisions", {
          firstEntry: { url: b.request.url, revision: b.revision },
          secondEntry: { url: a.request.url, revision: a.revision }
        });
    }
    _isAlreadyCached(a) {
      var b = this;
      return asyncToGenerator(function*() {
        const c = yield b._revisionDetailsModel.get(a.entryID);
        if (c !== a.revision) return !1;
        const d = yield b._getCache(),
          e = yield d.match(a.request);
        return !!e;
      })();
    }
    _onEntryCached(a) {
      var b = this;
      return asyncToGenerator(function*() {
        yield b._revisionDetailsModel.put(a.entryID, a.revision);
      })();
    }
    _onEntryDeleted(a) {
      var b = this;
      return asyncToGenerator(function*() {
        yield b._revisionDetailsModel.delete(a);
      })();
    }
    _close() {
    ...
    install() {
      return super.install().then(a => {
        ...
        a.forEach(a => {
          a.wasUpdated
            ? b.push({ url: a.url, revision: a.revision })
            : c.push({ url: a.url, revision: a.revision });
        });
        const d = {};
        return (
          0 < b.length &&
            (d["New / Updated Precache URL's"] = this._createLogFriendlyString(
              b
            )),
          0 < c.length &&
            (d["Up-to-date Precache URL's"] = this._createLogFriendlyString(c)),
          logHelper.log({
            message:
              `Precache Details: ${b.length} requests ` +
              `were added or updated and ` +
              `${c.length} request are already ` +
  ...
  class WorkboxSW$1 {
    constructor(
      ...
      if ("undefined" != typeof e)
        if (!1 === e || null === e) e = !1;
        else if ("string" != typeof e || 0 === e.length)
      ...
      f &&
        h.push(
          new BroadcastCacheUpdatePlugin({
            channelName: f,
            source:
              registration && registration.scope ? registration.scope : location
          })
        ),
        ...
        (this._precacheRouter = new Router$$1(
        ...
        this._registerInstallActivateEvents(b, c),
        this._registerDefaultRoutes(g, e);
    }
    precache(a) {
      if (!Array.isArray(a))
    ...
    _registerDefaultRoutes(a, b) {
      ...
      this._precacheRouter.registerRoute(({ url: c }) => {
        c.hash = "";
        const d = this._revisionedCacheManager.getCachedUrls();
        if (-1 !== d.indexOf(c.href)) return !0;
        let e = this._removeIgnoreUrlParams(c.href, a);
        return (
          -1 !== d.indexOf(e.href) ||
          (b &&
            e.pathname.endsWith("/") &&
            ((e.pathname += b), -1 !== d.indexOf(e.href)))
        );
      }, d);
    }
    _getCacheMatchPlugin(a, b) {
      ...
      const d = (() => {
        var d = asyncToGenerator(function*({
          ...
          if (f) return f;
          let h = c._removeIgnoreUrlParams(d.url, a);
          return e.match(h.toString(), g).then(function(a) {
            return !a && h.pathname.endsWith("/")
